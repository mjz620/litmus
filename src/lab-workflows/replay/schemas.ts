...
export const genericTraceProvenanceSchema = z.strictObject({
  ...
  validatorVersion: id,
  registrySnapshots: z
    .array(z.strictObject({ registryId: id, snapshotId: id }))
    .max(64),
  ...
  compatibility: z
    .strictObject({
      ...
      runtimeAdapterId: id,
      runtimeAdapterVersion: id,
      engineId: id,
      engineVersion: id,
      experimentDefinitionId: id,
      experimentDefinitionVersion: id
    })
    .nullable()
});
...
export const genericLabActionTraceSchema = z
  .strictObject({
    ...
    traceId: id,
    sessionId,
    sessionSeed: z.string().trim().min(1).max(256),
