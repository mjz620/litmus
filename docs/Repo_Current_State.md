...
`LC2-910`–`LC2-913` add native calorimetry on that same path: verified pour/mix/lid/probe and balance mechanics, deterministic thermal energy, and mass-derived dissolution enthalpy. Guest practice at `/lab/calorimetry` now defaults to the tared ammonium-nitrate dissolution workflow; the hot/cold-water workflow remains available as a Composer template.
