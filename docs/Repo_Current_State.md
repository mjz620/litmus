...
The next normal implementation ticket is `LC2-204`, semantic event envelopes and diagnosis integration. `LC2-203` completed exhaustive deterministic v2 condition evaluation, partial-order diagnoses, and generic workflow completion/failure status; `LC2-202` completed dependency-ordered chemistry-module coordination with test-only providers. Production acid-base support still belongs to `LC2-300`. Follow [`docs/lab-composer/README.md`](lab-composer/README.md) and the exact [`LC2-*` ticket backlog](lab-composer/tickets/README.md). Do not execute T0210–T0220 as written. Precipitation remains a static exact-ID plugin and is not automatically Composer support.
