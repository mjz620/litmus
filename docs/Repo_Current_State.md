...
The next normal implementation ticket is `LC2-205`, normalized action traces and deterministic replay. `LC2-204` completed deterministic semantic event envelopes, diagnosis evidence links, exact action/equipment/material references, and legacy StudentModel/coach/checkpoint adapters without changing the legacy event payload or persistence schema. `LC2-203` completed partial-order evaluation and workflow outcomes; `LC2-202` completed test-only chemistry-module coordination. Production acid-base support still belongs to `LC2-300`. Follow [`docs/lab-composer/README.md`](lab-composer/README.md) and the exact [`LC2-*` ticket backlog](lab-composer/tickets/README.md). Do not execute T0210–T0220 as written.
