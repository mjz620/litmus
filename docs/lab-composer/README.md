...
Phase 2's generic-runtime foundation is complete through `LC2-205`, and Phase 3 is implemented through `LC2-302`. The generic coordinator now has an explicit atomic legacy-runtime port selected by `compatibility.runtimeAdapterId`, never `familyId`. The port carries strict serialized titration state, calls the existing `ExperimentDefinition.step()` once, projects a conserved seeded ledger and engine-owned observables/ground truth, and returns unchanged legacy events for v2 envelopes and diagnoses. The exact deterministic v1 migration is checked in as an unvalidated v2 JSON source with pinned v1 content/validation hashes, migration provenance, registry snapshots, and v2 canonical hash. Its loader always performs current deterministic validation, and equivalence tests cover strict state, event, diagnosis, precedence, replay, and stale-artifact behavior. The existing student store now has an explicit `?runtime=setup-v2` development/compatibility path for that exact workflow and hash; default, invalid, retry, and precipitation routes remain legacy. Generic actions stay synchronous while the unchanged checkpoint and coach consumers run non-blocking, and the development inspector shows workflow, adapter, model, event, and diagnosis provenance. Canonical hashing is now synchronous platform-neutral SHA-256 so runtime validation remains local in the browser with all pinned hashes unchanged. The next ticket is `LC2-303`: derive the titration scene and visible controls from the validated setup/action projection. Production defaults remain unchanged.
