...
Phase 2's generic-runtime foundation is complete through `LC2-205`, and Phase 3 is implemented through `LC2-301`. The generic coordinator now has an explicit atomic legacy-runtime port selected by `compatibility.runtimeAdapterId`, never `familyId`. The port carries strict serialized titration state, calls the existing `ExperimentDefinition.step()` once, projects a conserved seeded ledger and engine-owned observables/ground truth, and returns unchanged legacy events for v2 envelopes and diagnoses. The exact deterministic v1 migration is checked in as an unvalidated v2 JSON source with pinned v1 content/validation hashes, migration provenance, registry snapshots, and v2 canonical hash. Its loader always performs current deterministic validation, and equivalence tests cover strict state, event, diagnosis, precedence, replay, and stale-artifact behavior. The next ticket is `LC2-302`: expose this definition through a development-only setup-driven session/store path that defaults to legacy. Production acid-base truth and the default student route remain unchanged.
