...
Phase 2's generic-runtime foundation is complete through `LC2-205`, and Phase 3 is implemented through `LC2-300`. The generic coordinator now has an explicit atomic legacy-runtime port selected by `compatibility.runtimeAdapterId`, never `familyId`. The port carries strict serialized titration state, calls the existing `ExperimentDefinition.step()` once, projects a conserved seeded ledger and engine-owned observables/ground truth, and returns unchanged legacy events for v2 envelopes and diagnoses. The next ticket is `LC2-301`: check in the deterministic migrated v2 definition and freeze v1/v2 equivalence. Production acid-base truth and the default student route remain unchanged.
