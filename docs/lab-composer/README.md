...
The next unblocked implementation ticket is `LC2-205`, versioned normalized action traces and deterministic replay. `LC2-204` now wraps unchanged legacy semantic payloads in deterministic session-scoped envelopes with monotonic event/action sequences, exact action/equipment/material references, and diagnosis links; pure adapters preserve StudentModel, coach, and checkpoint behavior. `LC2-203` provides exhaustive condition evaluation and workflow outcomes, while `LC2-202` provides test-only deterministic module coordination. Production acid-base compatibility remains reserved for `LC2-300`. Do not start UI or agent work before the remaining generic runtime prerequisites.
